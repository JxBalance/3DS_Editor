...
public class DesktopStartUI_T2 : MonoBehaviour
{

    public static DesktopStartUI_T2 _instance;
	...
	{
        //测试用默认IP
	    if (GameManagerT2._instance.nameStr != "")
	    {
            titleText.text = GameManagerT2._instance.nameStr;
	    }
        ipInput.text = "127.0.0.1";
        List<string> crewNames = new List<string>();
	    foreach (var c in GameManagerT2._instance.crew)
	    {
    ...
    {
        //开始进行
        GameManagerT2._instance.currentCrewIndex = crewDropdown.value;
        //是否启动多人协同
        if (gmsToggle.isOn)
        {
        ...
        //UI切换
        UIControllerT2._instance.ActivePanel((OperateType)operateDropDown.value);
        //相机切换
        GameManagerT2._instance.ActiveCamera((OperateType)operateDropDown.value);
    }
}
